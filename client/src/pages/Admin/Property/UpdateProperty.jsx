...
export default function UpdateProperty() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setValue('area', property.area[0]._id)
        setValue('developer', property.developer[0]._id)
        setValue('compound', property.compound[0]._id)
        setValue('type', property.type[0]._id)
      } catch (error) {
  ...
  return (
    <Container>
      <h3 className=" fs-3">Update Property</h3>
      <Form onSubmit={handleSubmit(onSubmit)}>
          ...
          {oldImages &&
            property?.images?.map((item, index) => (
              <img
                ...
                alt="images"
                className="img-fluid mb-3 shadow rounded-2 object-fit-cover"
        ...
        <Row className=" p-3">
          {oldThumbnail &&
            property?.thumbnail?.map((item, index) => (
              <img
                key={index + 1}
                src={`${import.meta.env.VITE_IMAGE_ORIGIN}/${item.url}`}
                alt="thumbnail"
        ...
        <LoadingButton loading={buttonLoading} text={'Update Property'} />
      </Form>
    </Container>
  )
}
