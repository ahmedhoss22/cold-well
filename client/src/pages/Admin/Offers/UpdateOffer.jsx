...
export default function UpdateOffer() {
  ...
  const fetchData = async () => {
    try {
      ...
      const offerResponse = await Api.get(`/offers/get-one/${id}`)
      setOffer(offerResponse.data.data)
      const offerData = offerResponse.data.data
      setValue('offerNumber', offerData.offerNumber)
      setValue('downPayment', offerData.downPayment)
      setValue('installmentYears', offerData.installmentYears)
      setValue('referenceNumber', offerData.referenceNumber)
      setValue('whatsapp', offerData.whatsapp)
      setValue('offerNote.en', offerData.offerNote?.en)
      setValue('offerNote.ar', offerData.offerNote?.ar)
      setValue('offerName.en', offerData.offerName.en)
      setValue('offerName.ar', offerData.offerName.ar)

      if (offerData.oldOffer) {
        setValue('oldOffer.downPayment', offerData.oldOffer.downPayment)
        setValue(
          'oldOffer.installmentYears',
          offerData.oldOffer.installmentYears
        )
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (offer) {
    setValue('developer[0]', offer.developer[0]._id)
    }
  },[offer])
  const onSubmit = async (data) => {
    try {
      await Api.put(`/offers/update-one/${id}`, data)
      ...
      notifyError('Failed to update offer!!')
  ...
  return (
    <Container>
      <h3 className=" fs-3">Update Offer</h3>
